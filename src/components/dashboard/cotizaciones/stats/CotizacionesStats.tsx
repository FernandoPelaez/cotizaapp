...
import { CheckCircle2, Clock3, FileText, Send, TimerReset, XCircle } from "lucide-react"
...
const stats: StatItem[] = [
  {
    ...
    icon_wrap: "bg-neutral-100 text-neutral-500",
  ...
  },
  {
    ...
    icon_wrap: "bg-slate-100 text-slate-500",
...
export default function CotizacionesStats({ summary }: CotizacionesStatsProps) {
  return (
    <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-6">
      {stats.map(({ key, label, description, icon: Icon, card, icon_wrap, badge, value, dot }) => (
        <div
          key={key}
        ...
        >
          {/* Dot decorativo */}
          <span className={`absolute right-2.5 top-2.5 h-1.5 w-1.5 rounded-full ${dot}`} />

          {/* Ícono + badge */}
          <div className="flex items-center gap-1.5">
            <div className={`inline-flex h-7 w-7 items-center justify-center rounded-md ${icon_wrap}`}>
              <Icon className="h-3.5 w-3.5" strokeWidth={1.8} />
            </div>
            <span className={`rounded px-1.5 py-px text-[9px] font-semibold uppercase tracking-widest ${badge}`}>
              {label}
            </span>
          </div>
          ...
          <div className="mt-2">
            ...
            <p className={`mt-0.5 text-xl font-semibold tabular-nums tracking-tight ${value}`}>
              {summary[key]}
            </p>
          </div>
        </div>
